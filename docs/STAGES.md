...
| Azure SNP (tested DCasv5 path) | Azure vTOM/paravisor + vTPM/MAA path; not supported by current raw collector | Not yet verified by attested-workload |
...
- **GitHub Action**: `maceip/unified-quote/action` tested on real workflow, correctly refuses on non-TEE runners
- **One-command verify**: `aw verify --remote https://<value_x>.aeon.site` from any machine
