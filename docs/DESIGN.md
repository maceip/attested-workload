...
- `aw check` should not re-verify the full quote chain on every
...
- Policy gates (KMS, Vault, the future `aw-gate`) follow the
  same pattern. Expensive check admits a key to a cache; cheap
  signature check grants access afterwards.
- This is how a attested-workload-style agent network makes thousands of
...
**How this shapes our work:** `aw-gate` (the policy evaluator)
...
- `aw check` verifies both the attested-TLS attestation (primary)
...
- First `aw check` for a domain: full quote chain verification,
...
- Same pattern applies to `aw-gate` (KMS / Vault / future
...
4. Attested-TLS verifier in `aw check`: cert → extension → EAT
...
11. `aw-gate` extraction: pull the policy evaluator out of
