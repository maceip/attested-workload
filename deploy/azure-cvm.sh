...
# ===========================================================================
# Deploy an Azure Confidential VM for attested-workload hardware validation.
#
# Prerequisites:
...
#   AZURE_RESOURCE_GROUP=aw-tee-validation
#   VM_NAME=aw-azure-snp
...
RESOURCE_GROUP="${AZURE_RESOURCE_GROUP:-aw-tee-validation}"
VM_NAME="${VM_NAME:-aw-azure-snp}"
...
echo "  scp v2/target/release/aw ${ADMIN_USERNAME}@${PUBLIC_IP}:~/aw"
...
echo "      and configfs-tsm report creation fails, attested-workload needs an Azure"
echo "      MAA/vTOM collector before stage0/stage1 can be marked verified."
echo ""
echo "  sudo ~/aw build ~/source --cmd 'echo stage0 build' --output ~/out"
echo "  sudo ~/aw run ~/source --attestation ~/out/attestation.cbor"
