...
docker run -d \
    ...
    -e RUNNER_NAME="attested-workload-tdx-$(hostname | cut -c1-8)" \
    -e RUNNER_LABELS="self-hosted,attested-workload,tee,tdx" \
    "${RUNNER_IMAGE}"
...
echo "    gcloud compute ssh ${INSTANCE_NAME} --zone=${ZONE} -- docker logs aw-runner"
echo ""
echo "  To tear down:"
