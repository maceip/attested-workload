...
/// TCP loopback port the in-enclave matcher/mailbox workload listens on. The
/// app-proxy forwards application requests (e.g. /v1/*) here. Distinct from
/// VSOCK_PORT (vsock namespace) even though both are 9384-ish; kept separate so
/// the workload and the attested-TLS server don't collide.
pub const MATCHER_LOOPBACK_PORT: u16 = 8080;

/// Forward an already-decrypted HTTP request to the loopback workload and stream
/// the response back through `out` as bytes arrive. Streaming (not read-to-EOF)
/// so Server-Sent-Events endpoints like /v1/deliver (text/event-stream) relay
/// live, while ordinary request/response endpoints (/v1/match, /v1/routing-key,
/// /v1/relay-path) work too — the workload speaks HTTP/1.0 and closes after the
/// body, which ends the loop. A read timeout bounds a hung upstream.
fn proxy_stream<W: std::io::Write>(request: &[u8], out: &mut W) -> std::io::Result<()> {
    let mut upstream =
        std::net::TcpStream::connect(("127.0.0.1", MATCHER_LOOPBACK_PORT))?;
    upstream.set_read_timeout(Some(std::time::Duration::from_secs(60)))?;
...
pub fn setup_loopback() -> Result<()> {
    ...
    match status {
        Ok(s) if s.success() => {
            eprintln!("[aw/vsock] Loopback interface up");
            Ok(())
        }
        _ => {
            ...
            match status2 {
                Ok(s) if s.success() => {
                    let _ = std::process::Command::new("ip")
                        ...
                        .status();
                    eprintln!("[aw/vsock] Loopback interface up (via ip)");
                    Ok(())
                }
                _ => {
                    eprintln!("[aw/vsock] WARNING: could not set up loopback");
...
pub fn bridge_vsock_to_loopback(loopback_port: u16) -> Result<()> {
    ...
    eprintln!("[aw/vsock] Bridge listening: vsock:{VSOCK_PORT} → 127.0.0.1:{loopback_port}");
    ...
    loop {
        ...
        if client_fd < 0 {
            eprintln!("[aw/vsock] Accept failed");
            continue;
        }
        ...
        std::thread::spawn(move || {
            if let Err(e) = pipe_vsock_to_tcp(client_fd, loopback_port) {
                eprintln!("[aw/vsock] Pipe error: {e}");
            }
        });
    }
}
...
pub fn bridge_tcp_to_vsock(listen_port: u16, enclave_cid: u32) -> Result<()> {
    ...
    eprintln!("[aw/vsock] Proxy listening: TCP:{listen_port} → vsock CID {enclave_cid}:{VSOCK_PORT}");

    for stream in listener.incoming() {
        let stream = match stream {
            Ok(s) => s,
            Err(e) => {
                eprintln!("[aw/vsock] TCP accept error: {e}");
                continue;
            }
        };
        ...
        std::thread::spawn(move || {
            if let Err(e) = pipe_tcp_to_vsock(stream, cid) {
                eprintln!("[aw/vsock] Proxy pipe error: {e}");
            }
        });
    }
...
pub fn serve_vsock(attestation_json: &str) -> Result<()> {
    let fd = create_vsock_listener()?;
    eprintln!("[aw/vsock] Serving attestation on vsock port {VSOCK_PORT}");
...
pub fn serve_tls_vsock(
    ...
    let fd = create_vsock_listener()?;
    // The app-proxy forwards to the workload on 127.0.0.1, so loopback must be up
        ...
        std::thread::spawn(move || {
            ...
            if path.starts_with("/v1/") || path == "/healthz" {
                if let Err(e) = proxy_stream(&buf[..total], &mut tls) {
                    let body = format!("matcher proxy error: {e}");
                    let msg = format!(
                    ...
                    let _ = tls.write_all(msg.as_bytes());
                }
                let _ = tls.conn.send_close_notify();
                let _ = tls.flush();
                return;
...
fn handle_kms_attestation(kms_state: &Option<std::sync::Arc<KmsState>>) -> String {
    ...
    match state.nsm.fresh_attestation(&state.report_data, &state.rsa_pub_der) {
        Ok(doc) => {
            ...
            eprintln!("[aw/vsock] Fresh attestation: {} bytes", doc.len());
        ...
        Err(e) => {
            eprintln!("[aw/vsock] Fresh attestation failed: {e}");
...
fn swap_tls_config(
    ...
    match config_result {
        ...
        Err(e) => {
            eprintln!("[aw/vsock] TLS cert swap failed: {e}");
    ...
    #[cfg(feature = "nitro")]
    {
        match crate::tee::nitro::kms_decrypt(kms_key, &ciphertext) {
            Ok(plaintext) => {
                ...
                eprintln!("[aw/vsock] KMS unwrap: {} bytes decrypted", plaintext.len());
                http_response(200, &b64)
            }
            Err(e) => {
                eprintln!("[aw/vsock] KMS unwrap failed: {e}");
