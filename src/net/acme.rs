...
pub async fn provision_cert_for_enclave(
    ...
    eprintln!("[aw/acme] Requesting cert for: {domain}");
    ...
    let (account, _credentials) = Account::builder()?
        ...
        .await?;
    eprintln!("[aw/acme] Account created");

    // Step 2: Create order
    let identifiers = vec![Identifier::Dns(domain.to_string())];
    let mut order = account.new_order(&NewOrder::new(&identifiers)).await?;
    eprintln!("[aw/acme] Order created");
    ...
    while let Some(result) = authorizations.next().await {
        ...
        eprintln!("[aw/acme] Challenge for {}", challenge.identifier());
        ...
        eprintln!("[aw/acme] Installing challenge cert into enclave...");
        ...
        eprintln!("[aw/acme] Challenge cert installed");

        // Tell Let's Encrypt we're ready
        challenge.set_ready().await?;
        eprintln!("[aw/acme] Challenge submitted, waiting for validation...");
    }
    ...
    eprintln!("[aw/acme] Challenge PASSED");

    // Step 5: Finalize — get the real cert
    let private_key_pem = order.finalize().await?;
    let cert_chain_pem = order.poll_certificate(&RetryPolicy::default()).await?;
    eprintln!("[aw/acme] Certificate issued for {domain}");
    ...
    eprintln!("[aw/acme] Real cert installed. TLS is now valid for {domain}");
    eprintln!("[aw/acme] Cert will appear in Certificate Transparency logs.");
