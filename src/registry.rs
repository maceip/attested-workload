...
impl TrustRoot {
    ...
    /// This exists so `aw check` of our own runner works out of
    /// the box without a config file.
    pub fn cvm_agent_default() -> Self {
...
impl Registry {
    ...
    /// `aw check` takes when no config is provided.
    pub fn load_default() -> anyhow::Result<Self> {
        let trust_root = TrustRoot::cvm_agent_default();
        let mut merged = Self {
            entries: HashMap::new(),
    ...
    fn check_sidecar(
        ...
        let sidecar = match sigverify::Sidecar::parse(&raw) {
            ...
            None => {
                eprintln!(
                    "[aw] Registry: {} present but unparseable",
                    sig_path.display()
                );
