//! attested-workload (`aw`): attested builds and runtime inside cloud TEEs.
//!
//! See docs/CONSTITUTION.md and docs/INVARIANT.md.
//!
        ...
        "run" => {
            ...
            match rt {
                ...
                Err(_) => {
                    ...
                    eprintln!("[aw] Async runtime unavailable, using sync mode");
                    cmd_run_sync(&args[2..])
                }
            }
        }
...
fn print_usage() {
    let bin = cli_name();
    eprintln!("{bin} — attested workload engine (build, run, check, enclave, proxy)");
    eprintln!();
    eprintln!("Usage:");
...
fn cmd_proxy(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] Proxy: TCP:{port} → enclave CID {cid}");
    eprintln!("[aw] TLS terminates inside the enclave. This proxy only sees encrypted bytes.");
...
fn cmd_build(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] TEE: {:?}", tee_provider.platform());
    ...
    eprintln!("[aw] Computing source hash (CT)...");
    let ct = compute_tree_hash(&source_dir)?;
    eprintln!("[aw] CT = {}", hex::encode(ct));
    ...
    eprintln!("[aw] Source frozen: {}", frozen_source.display());
    ...
    if is_cargo && !custom_cmd {
        ...
        eprintln!("[aw] Fetching Rust dependencies...");
    ...
    eprintln!("[aw] Ratchet verified: source unchanged after build");
    ...
    let dt: Option<[u8; 48]> = if dep_cache.exists() {
        match compute_tree_hash(&dep_cache) {
            Ok(h) if h != [0u8; 48] => {
                eprintln!("[aw] DT (dependencies): {}", hex::encode(h));
                Some(h)
            }
            _ => None,
        }
    ...
    eprintln!("[aw] Computing artifact hash (A)...");
    ...
    eprintln!("[aw] Computing Value X...");
    let value_x = compute_tree_hash(&frozen_source)?;
    eprintln!("[aw] X = {}", hex::encode(value_x));
    ...
    eprintln!("[aw] EAT binding: {}", hex::encode(binding));
    ...
    let tpm_attestation: Option<tee::tpm::TpmAttestation> =
        if tee_provider.platform() == quote::Platform::SevSnp && tee::tpm::tpm_available() {
            match tee::tpm::collect_tpm_attestation(&binding) {
                Ok(att) => {
                    eprintln!("[aw] NitroTPM: {} ", att.method);
                    eprintln!("[aw] NitroTPM digest: {}", hex::encode(att.digest));
                    for (i, pcr) in att.pcrs.iter().enumerate() {
                        eprintln!("[aw]   PCR{i}: {}", hex::encode(pcr));
                    }
                    Some(att)
                }
                Err(e) => {
                    eprintln!("[aw] WARNING: NitroTPM collection failed: {e}");
                    None
                }
            }
        } else {
    ...
    eprintln!("[aw] Collecting TEE attestation...");
    let evidence = tee_provider.collect_evidence(&report_data)?;
    eprintln!(
        "[aw] Quote collected: {} bytes from {:?}",
    ...
    if let Some(ref m) = platform_measurement {
        eprintln!("[aw] Platform measurement: {}", hex::encode(m));
    } else {
        eprintln!("[aw] WARNING: could not extract platform measurement from quote");
    }
    ...
    if log_committed {
        eprintln!("[aw] Transparency log: attestation committed to git");
    } else {
        eprintln!(
            "[aw] Transparency log: no git repo found (attestation written to disk only)"
        );
    }

    eprintln!();
    eprintln!("[aw] === Attested Build Complete ===");
    eprintln!("[aw] CT (source):   {}", hex::encode(ct));
    if let Some(ref d) = dt {
        eprintln!("[aw] DT (deps):     {}", hex::encode(d));
    }
    eprintln!("[aw] A  (artifact): {}", hex::encode(a));
    eprintln!("[aw] X  (value x):  {}", hex::encode(value_x));
    eprintln!("[aw] Platform:      {:?}", evidence.platform);
    eprintln!("[aw] Output:        {}", output_dir.display());
    eprintln!();
    eprintln!("[aw] This source became this artifact, inside genuine hardware.");
...
fn cmd_check(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] === attested-TLS check ===");
    eprintln!("[aw] Target: {host}:{port}");
    ...
    eprintln!("[aw] EAT profile: {}", eat.eat_profile);
    eprintln!("[aw] Platform:    {:?}", eat.platform_enum());
    eprintln!("[aw] Value X:     {}", hex::encode(eat.value_x));
    ...
    eprintln!("[aw] Verifying platform quote (binding + signature)...");
    match quote::verify::verify_platform_quote(platform, quote, &binding) {
        Ok(measurements) => {
            eprintln!("[aw] Quote binding:   PASS");
            eprintln!("[aw] Quote signature: PASS");
            for (name, val) in &measurements {
                eprintln!("[aw]   {}: {}", name, hex::encode(val));
            }
        }
        Err(e) => {
            eprintln!("[aw] Quote verify:    FAIL — {e}");
    ...
    if eat.has_previous() {
        ...
        {
            eprintln!(
                "[aw] Chain step {depth}: verifying previous stage ({} bytes EAT)",
            ...
            match quote::verify::verify_platform_quote(
                ...
                Ok(_) => {
                    eprintln!("[aw]   ✓ step {depth} quote verifies (Value X stable)");
                }
                Err(e) => {
        ...
        eprintln!("[aw] Chain:           PASS ({depth} stage(s) walked)");
    } else {
        eprintln!("[aw] Chain:           leaf only (no previous stage)");
    }
        ...
        Ok(scts) => match &issuer_der_opt {
            Some(issuer_der) => {
                ...
                if !report.any_verified() {
                    eprintln!("[aw]   WARNING: no SCTs from pinned logs — consider expanding net::ct::PINNED_LOGS");
                }
            }
            None => {
                eprintln!(
                    "[aw] CT (SCTs):       {} present but issuer cert not in chain — cannot verify",
                    scts.len()
                );
            }
        },
        Err(e) => {
            eprintln!("[aw] CT (SCTs):       malformed extension: {e}");
    ...
    match registry::Registry::load_default() {
        ...
        Ok(_) => {
            eprintln!("[aw] Registry: empty (no entries loaded)");
        }
        Err(e) => {
            eprintln!("[aw] Registry: load failed — {e}");
        }
    }

    eprintln!();
    eprintln!("[aw] === Check Complete ===");
    eprintln!(
        "[aw] {} is a genuine {:?} TEE running Value X {}",
        host,
        platform,
        &value_x_hex[..16]
    );
    ...
    if json_out {
        let out = serde_json::json!({
            ...
            "value_x": value_x_hex,
            "tls_spki_hash": hex::encode(actual_spki_hash),
            "verified": true,
        });
...
fn verify_attestation_json(
    ...
    eprintln!("[aw] === Verification ===");
    eprintln!("[aw] Platform: {platform_str}");
    eprintln!("[aw] CT: {ct_hex}");
    eprintln!("[aw] A:  {a_hex}");
    eprintln!("[aw] X:  {x_hex}");
    ...
    if expected_binding != binding_hex {
        eprintln!("[aw] FAIL: binding hash mismatch");
        eprintln!("[aw]   expected: {expected_binding}");
        eprintln!("[aw]   got:      {binding_hex}");
        std::process::exit(1);
    }
    eprintln!("[aw] Binding hash: PASS");
    ...
    if report_data_ok {
        ...
        eprintln!("[aw] Quote binding: FAIL (report_data doesn't match)");
        std::process::exit(1);
    }
        ...
        if let Some(p) = platform {
            if let Some(extracted) = extract_platform_measurement(&quote_bytes, &p) {
                ...
                if extracted_hex == platform_measurement_hex {
                    eprintln!("[aw] Platform measurement: PASS — matches attestation");
                } else {
                    eprintln!("[aw] Platform measurement: FAIL");
                    eprintln!("[aw]   attestation: {platform_measurement_hex}");
                    eprintln!("[aw]   extracted:   {extracted_hex}");
                    std::process::exit(1);
                }
            } else {
                eprintln!("[aw] Platform measurement: COULD NOT EXTRACT from quote");
                std::process::exit(1);
            }
        }
    } else {
        eprintln!("[aw] Platform measurement: NOT PRESENT in attestation");
        eprintln!("[aw] WARNING: cannot verify builder identity without platform measurement");
    }

    // Check 4: Verify TEE quote signature chain
    // This is the cryptographic proof that the quote is from real hardware.
    eprintln!("[aw] Verifying TEE signature chain...");
    let platform = match platform_str {
        "Tdx" => Some(quote::Platform::Tdx),
        "SevSnp" => Some(quote::Platform::SevSnp),
        "Nitro" => Some(quote::Platform::Nitro),
    ...
    if let Some(p) = platform {
        ...
        match quote::verify::verify_platform_quote(p, &quote_bytes, &binding_arr) {
            Ok(measurements) => {
                eprintln!("[aw] TEE signature chain: PASS");
                for (name, val) in &measurements {
                    eprintln!("[aw]   {}: {}", name, hex::encode(val));
                }
            }
            Err(e) => {
                eprintln!("[aw] TEE signature chain: FAIL — {e}");
                ...
                eprintln!("[aw] WARNING: quote may not be from genuine hardware");
            }
        }
    }
    ...
    if let Some(dir) = source_dir {
        eprintln!("[aw] Verifying source hash against {}", dir.display());
        let local_ct = compute_tree_hash(dir)?;
        if hex::encode(local_ct) == ct_hex {
            eprintln!("[aw] Source hash: PASS — matches attestation");
        } else {
            eprintln!("[aw] Source hash: FAIL");
            eprintln!("[aw]   attestation: {ct_hex}");
            eprintln!("[aw]   local:       {}", hex::encode(local_ct));
            std::process::exit(1);
        }
    }
    ...
    if let Some(path) = artifact_path {
        eprintln!(
            "[aw] Verifying artifact hash against {}",
            path.display()
        );
        let bytes = std::fs::read(path)?;
        let local_a = hex::encode(Sha384::digest(&bytes));
        if local_a == a_hex {
            eprintln!("[aw] Artifact hash: PASS — matches attestation");
        } else {
            eprintln!("[aw] Artifact hash: FAIL");
            std::process::exit(1);
        }
    }
    ...
    eprintln!();
    eprintln!("[aw] === Verification Complete ===");
    eprintln!("[aw] This artifact was built from this source inside genuine {platform_str} hardware.");
...
async fn cmd_run(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] === Stage 1: Attested Runtime ===");
    ...
    match quote::verify::verify_platform_quote(
        ...
        Ok(measurements) => {
            eprintln!("[aw] Stage 0 quote: PASS");
            for (name, val) in &measurements {
                eprintln!("[aw]   {}: {}", name, hex::encode(val));
            }
        }
    ...
    eprintln!("[aw] Value X: MATCHES stage 0");
    ...
    eprintln!("[aw] Collecting stage 1 quote...");
    let evidence = tee_provider.collect_evidence(&report_data)?;
    eprintln!(
        "[aw] Stage 1 quote: {} bytes from {:?}",
        evidence.raw_quote.len(),
        evidence.platform
    );
    ...
    eprintln!(
        "[aw] Attested-TLS cert built ({} bytes DER) — serving at {}",
    ...
    // --- Step 10: serve ---
    eprintln!();
    eprintln!("[aw] === Stage 1 Verified ===");
    eprintln!("[aw] Chain: source → attested build → attested runtime");
    eprintln!("[aw] Value X: {}", hex::encode(current_x));
    ...
    tokio::spawn(async move {
        if let Err(e) = net::tls::serve(state_clone, 443).await {
            eprintln!("[aw] TLS server error: {e}");
        }
    });
    eprintln!("[aw] Attested TLS server started on :443");

    // --- Step 11: execute workload if provided ---
    if let Some(cmd) = run_cmd {
        eprintln!("[aw] Running: {cmd}");
        let status = std::process::Command::new("sh")
            .arg("-c")
            .arg(&cmd)
        ...
        eprintln!("[aw] No --cmd provided. Serving attestation.");
        eprintln!("[aw] Press Ctrl+C to stop.");
...
fn cmd_enclave(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] Enclave mode: build + serve in one process");
    ...
    eprintln!("[aw] Generating attested-TLS keypair inside enclave");
    let tls_kp = net::attested_tls::generate_keypair()?;
    let tls_spki_hash = net::attested_tls::spki_hash_of(&tls_kp);
    eprintln!("[aw] TLS SPKI sha256: {}", hex::encode(tls_spki_hash));
    ...
    let eat_cbor = eat.to_cbor()?;
    eprintln!(
        "[aw] EAT (CBOR): {} bytes — embedded in attested-TLS cert + served at /eat",
        eat_cbor.len()
    );

    eprintln!("[aw] === Enclave Ready ===");
    eprintln!("[aw] Value X: {}", hex::encode(value_x));
    eprintln!("[aw] Domain: {domain}");
    if kms_private_key.is_some() {
        eprintln!("[aw] KMS: GET /kms-attestation (fresh doc) + POST /kms-unwrap");
    }
    ...
    match rustls::crypto::ring::default_provider().install_default() {
        Ok(_) => {
            ...
            let tls_config = {
                ...
                eprintln!(
                    "[aw] attested-TLS cert built ({} bytes DER, EAT extension marked critical)",
                    ac.cert_der.len()
                );
                net::tls::make_server_config(ac.cert_pem.as_bytes(), ac.key_pem.as_bytes())?
            };
            eprintln!("[aw] TLS on vsock (inside enclave)");
            #[cfg(all(feature = "nitro", unix))]
            {
        ...
        Err(_) => {
            eprintln!("[aw] TLS crypto unavailable — serving plain HTTP on vsock");
...
fn cmd_run_sync(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] Stage 1 (sync mode): self-verification");
    ...
    eprintln!("[aw] Stage 0 Value X: {}", &stage0_x[..24]);
    ...
    if current_x_hex != stage0_x {
        eprintln!("[aw] FATAL: Value X mismatch");
        eprintln!("[aw]   stage 0: {stage0_x}");
        eprintln!("[aw]   current: {current_x_hex}");
        std::process::exit(1);
    }
    eprintln!("[aw] Value X: MATCHES");
    ...
    eprintln!("[aw] === Stage 1 Verified (sync) ===");
    eprintln!("[aw] Value X: {current_x_hex}");

    // Serve via vsock (blocking)
    let domain = net::acme::domain_from_value_x(&current_x);
    eprintln!("[aw] Domain: {domain}");
    eprintln!(
        "[aw] Serving via vsock on port {}",
...
fn cmd_merge(args: &[String]) -> anyhow::Result<()> {
    ...
    eprintln!("[aw] All attestations agree:");
    eprintln!("[aw]   Value X: {first_x}");
    eprintln!("[aw]   CT:      {first_ct}");
    eprintln!("[aw]   A:       {first_a}");
    ...
    eprintln!();
    eprintln!("[aw] === Merged Attestation ===");
    eprintln!("[aw] Platforms: {}", platforms_seen.join(", "));
    eprintln!("[aw] Value X: {first_x}");
    eprintln!("[aw] Output: {}", output_path.display());
    eprintln!();
    if platforms_seen.len() >= 2 {
        eprintln!(
            "[aw] Anytrust: {} independent TEE vendors attest the same Value X.",
            platforms_seen.len()
        );
        eprintln!("[aw] Trust at least one vendor → trust the build.");
    }
...
fn detect_build_cmd(dir: &Path) -> String {
    if dir.join("Cargo.toml").exists() {
        ...
        eprintln!("[aw] WARNING: no build system detected, using 'make'");
        "make".into()
    }
}
